      ...
      >
        <div className="overflow-hidden rounded-[30px] border border-purple-100/80 bg-white/95 p-4 shadow-[0_30px_80px_rgba(76,29,149,0.14)] backdrop-blur-2xl dark:border-purple-900/40 dark:bg-slate-950/95">
          <div className="grid grid-cols-[220px_minmax(0,1fr)_260px] gap-4">
            ...
            <div className="rounded-[24px] bg-gradient-to-br from-slate-900 via-slate-900 to-purple-950 p-5 text-white shadow-lg shadow-slate-900/20">
              <div className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">推荐发现入口</div>
                  ...
                  <Link
                    ...
                    href={buildDiscoveryHref({
                      ...
                      sort:
                        view.slug === 'ending'
                          ? 'deadline'
                          : view.slug === 'completion'
                            ? 'completion'
                            : view.slug === 'new'
                              ? 'newest'
                              : undefined,
                    })}
                    className="block rounded-2xl border border-white/10 bg-white/5 px-4 py-3 transition-all hover:border-white/20 hover:bg-white/10"
                  >
                    <div className="font-medium">{view.label}</div>
                    <div className="mt-1 text-xs leading-5 text-slate-300">{view.description}</div>
                  </Link>
                ))}
              </div>
              <Link
                href="/projects"
                className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-purple-200 transition-colors hover:text-white"
              >
                进入完整发现页
                <ArrowUpRight className="h-4 w-4" />
              </Link>
